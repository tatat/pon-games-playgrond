...
- **`onUpdate` only after `onEnter` resolves.** The ticker hands frames to a scene only once its `onEnter` (including any awaited preload) has fully resolved. Scenes never see an `onUpdate` while `this.input` or other resources are still being set up — and an in-flight `destroy()` blocks further updates immediately. Scene code can therefore use non-nullable fields populated in `onEnter` without defensive guards.
